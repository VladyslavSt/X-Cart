...
class XPaymentsCloud extends \XLite\Model\Payment\Base\CreditCard
{
    ...
    {
        return ModuleMain::isDelayedPaymentEnabled() && isset(\XLite\Core\Request::getInstance()->savedCardId)
            ? $this->doDelayedPaymentByCardId(\XLite\Core\Request::getInstance()->savedCardId)
            : $this->doPaymentByToken(\XLite\Core\Request::getInstance()->xpaymentsToken);
    }
     ...
     * @return string
     */
    protected function doDelayedPaymentByCardId(string $savedCardId)
    {
        $this->setXpidForTransaction($this->transaction, $savedCardId);
        $this->transaction->getOrder()->setDelayedPaymentSavedCardId($savedCardId);
        $this->transaction->setValue(0.00);

        return static::PENDING;
    }

    /**
     * @param string $xpaymentsToken
     *
     * @return string
     */
    protected function doPaymentByToken(string $xpaymentsToken)
    {
        $api = $this->initClient();
        ...
        try {
            if (ModuleMain::isDelayedPaymentEnabled()) {
     ...
     * @param string      $savedCardId
     */
    protected function setXpidForTransaction(Transaction $transaction, string $savedCardId)
    {
        try {
            $response = $this->initClient()->doGetCustomerCards(
                $transaction->getOrder()->getOrigProfile()->getXpaymentsCustomerId()
            );
            if (is_array($response->cards)) {
                foreach ($response->cards as $card) {
                    if ($savedCardId == $card['cardId']) {
                        $transaction->setXpaymentsId($card['xpid']);
                        break;
                    }
                }
            }
    ...
    public function doRebill(Order $order, float $amount, string $xpid)
    {
        ...
        $newTransaction->setPaymentMethod(ModuleMain::getPaymentMethod());
    ...
    protected function processPaymentFinish(Transaction $transaction, XpPayment $payment, $isRebill = false)
    {
        ...
        if (
            ModuleMain::isDelayedPaymentEnabled()
            && !$isRebill
        ) {
            if (
                isset($payment->card->saved)
                && true === $payment->card->saved
                && isset($payment->card->savedCardId)
            ) {
                $transaction->getOrder()->setDelayedPaymentSavedCardId($payment->card->savedCardId);
